...
class Modalityloss(nn.Module):
    def __init__(self):
        ...
        self.net = resnet18(spatial_size=128, sample_duration=128, num_classes=4)
        dir = '/cpfs01/projects-HDD/cfff-7abceac4e328_HDD/dhm_41310/chwang/Logs/pretrained/40_net.pth'
        print('initiaze the model from: {}'.format(dir))
        takeoff_statedict = torch.load(dir, map_location='cpu')
        self.net.load_state_dict(takeoff_statedict, strict=False)
        self.criterion=nn.CrossEntropyLoss()
        self.eval()
        self.net.eval()

    def forward(self, input, target) :
        # print(self.net(input),target)
        return self.criterion(self.net(input),target)
